...
*/
require_once get_theme_file_path().'/inc/richee-customizer.php';
/*
*Richee Demo Import 
*/
...
if ( ! defined( '_RICHEE_VERSION' ) ) {
	...
	define( '_RICHEE_VERSION', '1.0.0');//'1.0.0');
}
...
function richee_setup() {
	...
	register_nav_menus(
		array(
			'richee_primary' => esc_html__( 'Primary Menu', 'richee' ),
		)
	);
...
function richee_enqueue_scripts() {
	...
	wp_enqueue_style( 'richee-template-style', get_template_directory_uri().'/assets/css/style.css', array(),_RICHEE_VERSION);

	wp_enqueue_style( 'richee-google-fonts','//fonts.googleapis.com/css2?family=Poppins:wght@300;500;700&display=swap', array());

	wp_enqueue_script( 'comment-reply' );

	wp_enqueue_style( 'richee-style',get_stylesheet_uri(), array(),_RICHEE_VERSION,false);
	
	//JAVASCRIPT FILE ENQUEUE

	if('front-page-template.php' == basename(get_page_template())){

		wp_enqueue_script( 'richee-bootstrap', get_template_directory_uri().'/assets/js/bootstrap.min.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-animate-typing', get_template_directory_uri().'/assets/js/jquery.animateTyping.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-progressbar', get_template_directory_uri().'/assets/js/progress-bar.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-progressbar-circle', get_template_directory_uri().'/assets/js/progresscircle.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-onepage-nav', get_template_directory_uri().'/assets/js/onepageNav.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-owl-carousel', get_template_directory_uri().'/assets/js/owl.carousel.min.js', array('jquery'), _RICHEE_VERSION, true );

		wp_enqueue_script( 'richee-autofilter-js', get_template_directory_uri().'/assets/js/autofilter.js', array('jquery'), _RICHEE_VERSION, true );

	}
	wp_enqueue_script( 'richee-main-js', get_template_directory_uri().'/assets/js/main.js', array('jquery'), _RICHEE_VERSION, true );
